...
class Percettrone
{
	...
	public:
		...
		bool T(real* p_inputs)
		{
			return A(p_inputs) > m_threshold;
		}
		
		void Apprendimento(int p_n_examples, real* p_inputs, bool* p_answers, real p_eta, int p_epochs, real p_min_error)
		{
			int current_epoch=0;
			real current_error;
			real weights_correction[m_n_input];
			real threshold_correction;
			do
			{
				current_error = 0;
				fill(&weights_correction[0], &weights_correction[m_n_input], 0.0);
				threshold_correction = 0;
				for (int i=0; i<p_n_examples; i++)
				{
					real output = T(&p_inputs[i*m_n_input]);
					current_error += std::pow(p_answers[i] - output, 2)/2;
					for (int j=0; j<m_n_input; j++)
						weights_correction[j]+= (output - p_answers[i]) * p_inputs[i*m_n_input+j];
					threshold_correction += (output - p_answers[i]);
				}
				for (int i=0; i<m_n_input; i++)
					m_weights[i] = m_weights[i] - weights_correction[i] * p_eta / p_n_examples;
				m_threshold = m_threshold + threshold_correction * p_eta / p_n_examples;
				current_error /= p_n_examples;
...
int main(int argc, char* argv[]) {
	...
	if (type == 0)
	{
		...
		inputs =new real[24]{
			...
			1,1,0, //caso c
			1,1,1};//caso c
		risposte=new bool[8]{0,0,0,0,0,1,1,1};
	...
	else
	{
		...
		switch (type)
		{
			 /*AND*/ case 1: risposte=new bool[4]{0,0,0,1}; cout<<"AND"<<std::endl; break;
			 /*OR*/  case 2: risposte=new bool[4]{0,1,1,1}; cout<<"OR"<<std::endl; break;
			 /*XOR*/ case 3: risposte=new bool[4]{0,1,1,0}; cout<<"XOR"<<std::endl; break;
			 default: return(1);
		}
		for (int i=0; i<4; i++)
